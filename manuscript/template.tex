...
\subsection{The exact discrete partial likelihood as a fourth arm}
\label{sec:arm-exact}

Section~\ref{sec:arms} varies the baseline while holding the coefficient fixed, which isolates baseline estimation but leaves the Efron approximation itself untested. The estimator designed for intrinsically discrete failure times is Cox's exact discrete conditional partial likelihood, in which the baseline is removed by conditioning rather than by profiling,
\begin{equation}
L_t=\frac{\exp(\beta'S_{D_t})}{e_{d_t}\!\left(\{\exp(\beta'x_i):i\in R_t\}\right)},
\label{eq:exact-discrete}
\end{equation}
with $e_{d}$ the elementary symmetric polynomial of order $d$ and $S_{D_t}$ the sum of covariates over the failures in interval $t$. Adding it as a fourth arm gives a decomposition in which each successive step changes exactly one component:
\begin{equation}
\begin{split}
\text{A: }&\widehat\beta_E+\text{Breslow baseline},\\
\text{B: }&\widehat\beta_E+\text{cloglog profile baseline},\\
\text{C: }&\widehat\beta_X+\text{cloglog profile baseline},\\
\text{D: }&(\widehat\alpha,\widehat\beta)\text{ from joint grouped maximum likelihood},
\end{split}
\label{eq:four-arms}
\end{equation}
where $\widehat\beta_X$ maximises Eq.~\eqref{eq:exact-discrete}. Then $A-B$ is the baseline representation, $B-C$ is Efron against exact tie handling with the baseline held fixed, and $C-D$ is the residual. Arm C profiles its baseline under the complementary log-log link rather than the logistic one so that B and C differ in the coefficient alone.

Two costs must be reported honestly. The recursion for Eq.~\eqref{eq:exact-discrete} requires $n\left(\max_t d_t+1\right)$ sequential steps, and $\max_t d_t$ grows with $n$ at fixed tie concentration, so the cost is $O(n^2)$. Where a configuration exceeds the computational budget, every arm is refitted on the largest feasible subsample so that the comparison remains paired, and both blocks are reported rather than the feasible one alone.
...
\subsection{Exact tie handling does not explain the coefficient difference}
\label{sec:results-exact}

Across the 135 cells of the grouped-Weibull design, arm C is computable in 102; the remaining 33 exceed the recursion budget, all at $n=10{,}000$. Writing the attenuation of each estimator as the slope of its mean estimate on the true coefficient, so that 1.000 is unbiased and values below 1 are shrunk toward zero:

\begin{table}[htbp]
\centering
\caption{Coefficient attenuation under the three estimators, over the cells where the exact arm is computable. Values below 1 attenuate toward zero; values above 1 overshoot.}
\label{tab:attenuation}
\begin{tabular}{lccc}
\hline
Cells & Efron & Exact & Grouped \\
\hline
All 102 & 0.880 & 1.236 & 0.983 \\
$T\le 4$ (36) & 0.709 & 1.429 & 0.938 \\
\hline
\end{tabular}
\end{table}

Exact tie handling does not recover what the Efron approximation loses. It errs in the opposite direction and by a larger margin, and lies further from the true coefficient than the Efron estimate in 97 of 102 cells.

This is not a defect of the estimator. Scored against the joint maximum likelihood estimator of its own link family, arm C is indistinguishable from it: the median absolute difference in held-out negative log-likelihood is $2.7\times10^{-5}$, against $7.8\times10^{-3}$ for the Efron arm in the same family. Conditioning the $T$ nuisance parameters away therefore costs essentially nothing relative to estimating them, which also serves as a check on the implementation. The coefficient in Eq.~\eqref{eq:exact-discrete} is, however, a log odds ratio, and a log odds ratio exceeds in magnitude the log hazard ratio generating complementary log-log data. Arm C is not a poorer estimate of the same quantity but an estimate of a different one. Accordingly, arm C predicts worse than arm B in 102 of 102 cells when both are scored in the complementary log-log family, and in only 13 of 102 when each is scored in its own family; the first comparison is confounded with link choice and the second is not.

The same ordering holds on observed data. Over seven configurations in which the fitted coefficients were retained for every split, the exact coefficient is closer to the grouped estimate in none of the seven paired comparisons, and lies further from it than the Breslow estimate in six. On drsa/clinic at $T=50$, the single configuration whose training split falls within budget and therefore requires no subsampling, the relative distance $\lVert\widehat\beta_{\mathrm{arm}}-\widehat\beta_G\rVert/\lVert\widehat\beta_G\rVert$ is 0.0135 for the Efron arm and 0.133 for the exact arm.

Two further observations follow. The disagreement is a property of tie severity rather than a fixed property of the estimator: on drsa/clinic it falls by a factor of 10.3 between $T=5$ and $T=50$ as modal event mass declines from 0.837 to 0.404. And the exact method is least available where tie concentration is highest. On nested subsamples of sparcs/drg302 it remains computable to $n=2{,}000$ and no further, the full cohort exceeding the budget by a factor of 132, with the $O(n^2)$ growth confirmed to within 2\% across the rungs at near-constant modal event mass.
...
A second simulation checked a different possible interpretation. A continuous Weibull proportional hazards process was generated and then grouped exactly on the analysis grid. At $n=10{,}000$ without censoring and Weibull shape 0.8, the grouped coefficient bias stayed at or below 1.3\% for all tested grids. The Cox coefficient bias was 34.8\% at $T=2$, 21.0\% at $T=4$, 10.2\% at $T=8$, 3.1\% at $T=20$ and 1.3\% at $T=40$. The corresponding $D_B$ values were 0.17455, 0.10100, 0.04802, 0.01299 and 0.00442. Exactness of the grouped representation therefore does not imply equality of the two estimators after failure times have been tied. It guarantees correctness of the grouped complementary-log-log model under the simulated data-generating process.
...
One caveat applies to the repeated-split standard errors on the DRSA music records specifically. Splits are drawn over records, and the reported intervals treat records as independent. The distributed files carry no grouping identifier, so whether the records are independent units or repeated observations on a smaller number of units cannot be determined from them. If the latter, the intervals for that cohort are too narrow and should be read as lower bounds. The ordering result of Section~
ef{sec:event-sim} does not rest on this cohort: the rank correlation of 0.986 is computed over eighteen configurations drawn from four cohorts, and falls only to 0.969 when the four music configurations are removed.
